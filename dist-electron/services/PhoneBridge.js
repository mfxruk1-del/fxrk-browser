...
class PhoneBridge extends events_1.EventEmitter {
    browser = null;
    page = null;
    ...
    async connect(config) {
        if (this.status.connected || this.status.authenticating)
            return;
        ...
        try {
            ...
            if (!chromePath) {
                throw new Error('Chrome/Edge not found. Please install Google Chrome to use phone integration.');
            }
            logger_1.logger.phone.info(`Launching headless browser: ${chromePath}`);
            ...
            // Navigate to iCloud
            await this.page.goto(constants_1.ICLOUD_URLS.login, {
            ...
            DatabaseService_1.db.savePhoneConfig({ ...config, sessionCookies: cookies, lastConnected: Date.now() });
            ...
            this.startPolling();
            logger_1.logger.phone.info('Phone bridge connected successfully');
        }
        catch (err) {
            const error = err instanceof Error ? err.message : String(err);
            logger_1.logger.phone.error(`Connection failed: ${error}`);
    ...
    async performLogin(appleId, password) {
        if (!this.page)
            throw new Error('No page available');
        logger_1.logger.phone.info('Performing iCloud login...');
        ...
        if (appleIdInput) {
            ...
            const is2FA = await this.page.$('#char0, #verification-code-field, input[inputmode="numeric"]')
                ...
                .catch(() => false);
            if (is2FA) {
                logger_1.logger.phone.info('2FA required - waiting for code from user');
        ...
        logger_1.logger.phone.info('Login completed');
    }
    /** Navigate to iCloud Messages */
    async openMessages() {
        if (!this.page)
            throw new Error('No page');
        logger_1.logger.phone.info('Navigating to iCloud Messages...');
        await this.page.goto(constants_1.ICLOUD_URLS.messages, {
            waitUntil: 'networkidle2',
            timeout: 30000,
        });
        // Wait for message list to appear
        await this.page.waitForSelector('[class*="conversation"], [class*="message-list"], [class*="thread-list"], ul[role="listbox"]', { timeout: 20000 }).catch(() => {
            logger_1.logger.phone.warn('Message list selector not found — iCloud UI may have changed');
        });
    ...
    async loadConversations() {
        ...
        try {
            ...
            logger_1.logger.phone.info(`Loaded ${conversations.length} conversations`);
        }
        catch (err) {
            logger_1.logger.phone.warn(`Failed to load conversations: ${err}`);
        }
    }
    ...
    async setupMessageObserver() {
        if (!this.page)
            return;
        logger_1.logger.phone.info('Setting up MutationObserver for real-time message detection...');
        await this.page.exposeFunction('onNewMessageDetected', (sender, text, timestamp) => {
        ...
        logger_1.logger.phone.info('MutationObserver set up successfully');
    }
    /** Handle a newly detected message */
    handleNewMessage(sender, text, timestamp) {
        ...
        if (codeResult) {
            ...
            logger_1.logger.phone.info(`Verification code detected: ${codeResult} from ${sender}`);
    ...
    startPolling() {
        this.pollTimer = setInterval(async () => {
            if (!this.status.connected || !this.page)
                return;
            try {
                await this.loadConversations();
            }
            catch (err) {
                logger_1.logger.phone.warn(`Polling error: ${err}`);
                // Check if session expired
                const url = this.page?.url() || '';
                if (url.includes('/login')) {
                    logger_1.logger.phone.warn('Session expired — notifying user');
        ...
        }, constants_1.PHONE_POLL_INTERVAL);
    }
    stopPolling() {
        if (this.pollTimer) {
    ...
    async sendMessage(to, text) {
        ...
        try {
            ...
            await this.page.evaluate((recipient) => {
            ...
            const composeArea = await this.page.$('textarea[placeholder*="message"], textarea[placeholder*="iMessage"], [contenteditable="true"][class*="compose"]');
            if (!composeArea) {
            ...
            logger_1.logger.phone.info(`Sent message to ${to}: ${text.substring(0, 50)}`);
        }
        catch (err) {
            logger_1.logger.phone.error(`Failed to send message: ${err}`);
            throw err;
        }
    }
    /** Create an iCloud Note (to send content to iPhone) */
    async sendToPhone(type, content, title) {
        if (!this.page || !this.status.connected) {
            throw new Error('Phone bridge not connected');
        }
        ...
        try {
            ...
            await notesPage.goto(constants_1.ICLOUD_URLS.notes, { waitUntil: 'networkidle2', timeout: 20000 });
            ...
            const noteContent = await notesPage.$('[contenteditable="true"], textarea[class*="content"], [class*="note-content"]');
            if (noteContent) {
                await noteContent.click();
                await noteContent.type(noteTitle + '\n' + content, { delay: 20 });
                logger_1.logger.phone.info(`Created iCloud Note: "${noteTitle}"`);
            }
            ...
            logger_1.logger.phone.error(`Failed to create iCloud Note: ${err}`);
            throw err;
        }
    }
    /** Check iCloud Notes for content pushed from phone (clipboard sync) */
    async checkPhoneClipboard() {
        if (!this.browser || !this.status.connected)
            return null;
        try {
            const notesPage = await this.browser.newPage();
            await notesPage.goto(constants_1.ICLOUD_URLS.notes, { waitUntil: 'networkidle2', timeout: 15000 });
            const content = await notesPage.evaluate(() => {
    ...
    async restoreSession(cookiesJson) {
        if (!this.page)
            return;
        try {
            ...
            logger_1.logger.phone.info(`Restored ${cookies.length} session cookies`);
        }
        catch {
            logger_1.logger.phone.warn('Failed to restore session cookies');
        }
    }
    ...
    async disconnect() {
        ...
        this.emit('statusChange', this.status);
        logger_1.logger.phone.info('Phone bridge disconnected');
    }
    async cleanup() {
        try {
            if (this.page) {
            ...
            logger_1.logger.phone.warn(`Cleanup error: ${err}`);
        }
    }
}
exports.phoneBridge = new PhoneBridge();
exports.default = exports.phoneBridge;
//# sourceMappingURL=PhoneBridge.js.map